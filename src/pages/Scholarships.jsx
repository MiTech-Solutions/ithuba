...
const TYPES      = ["All types", "Merit", "Need-based", "Athletic", "Arts", "Community"];
...
export default function Scholarships() {
  ...
  const filtered = useMemo(() => {
    let results = scholarships.filter((s) => {
      ...
      const matchSearch = !q || s.name?.toLowerCase().includes(q) || s.funder?.toLowerCase().includes(q) || s.field_of_study?.toLowerCase().includes(q) || s.description?.toLowerCase().includes(q);
      const matchField  = field === "All fields"    || s.field_of_study?.toLowerCase().includes(field.toLowerCase());
      const matchType   = type === "All types"      || s.scholarship_type?.toLowerCase() === type.toLowerCase();
      const matchLevel  = level === "All levels"    || s.study_level?.toLowerCase().includes(level.toLowerCase());
      const matchProv   = province === "All provinces" || s.province?.toLowerCase().includes(province.toLowerCase()) || s.province?.toLowerCase() === "national";
    ...
    return results;
  }, [scholarships, search, field, type, level, province, sort]);

  const hasFilters = field !== "All fields" || type !== "All types" || level !== "All levels" || province !== "All provinces";

  function clearFilters() {
    setField("All fields"); setType("All types"); setLevel("All levels");
    setProvince("All provinces"); setSearch(""); setSort("default");
  }

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    "name": "South African Scholarships — Ithuba Directory",
    "description": "A directory of scholarships available to South African students.",
    "url": "https://ithubahub.co.za/scholarships",
    "numberOfItems": scholarships.length,
    "itemListElement": scholarships.map((s, i) => ({
      "@type": "ListItem",
      "position": i + 1,
      "name": s.name,
      "url": `https://ithubahub.co.za/scholarships/${slugify(s.name)}`,
    })),
  };
    ...
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="mt-2 font-display text-3xl font-semibold text-forest-900 dark:text-forest-50 sm:text-4xl">All scholarships</h1>
          <p className="mt-2 text-forest-600 dark:text-forest-400 text-sm">
            {loading ? "Loading..." : `${filtered.length} scholarships found`}
            {!loading && hasFilters && ` · filtered from ${scholarships.length} total`}
          </p>
        </div>
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-forest-200 dark:border-forest-700 bg-white dark:bg-forest-900 py-3.5 pl-10 pr-4 text-sm text-forest-900 dark:text-forest-100 outline-none placeholder:text-forest-400 focus:border-forest-500 dark:focus:border-forest-400 focus:shadow-lg focus:shadow-forest-500/10 focus:scale-[1.01] transition-all duration-200"
          />
          {search && (
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <select value={sort} onChange={(e) => setSort(e.target.value)} className="rounded-xl border border-forest-200 dark:border-forest-700 bg-white dark:bg-forest-900 px-3 py-2 text-sm text-forest-700 dark:text-forest-300 outline-none focus:border-forest-500 transition cursor-pointer">
              {SORT_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => setFiltersOpen(!filtersOpen)} className="inline-flex items-center gap-2 rounded-xl border border-forest-200 dark:border-forest-700 bg-white dark:bg-forest-900 px-4 py-2.5 text-sm font-medium text-forest-700 dark:text-forest-300 lg:hidden">
              <SlidersHorizontal size={15} /> Filters {hasFilters && <span className="h-2 w-2 rounded-full bg-gold-500" />}
            </button>
            {(hasFilters || sort !== "default") && (
              <button onClick={clearFilters} className="text-sm text-forest-500 dark:text-forest-400 hover:text-forest-800 dark:hover:text-white transition">Clear all</button>
            )}
          </div>
        </div>
        ...
        <div className="flex gap-6">
          ...
          {filtersOpen && (
            <div className="fixed inset-0 z-50 lg:hidden" onClick={() => setFiltersOpen(false)}>
              ...
              <div className="absolute left-0 top-0 bottom-0 w-72 bg-cream dark:bg-forest-950 shadow-2xl flex flex-col animate-[slideInLeft_0.3s_ease]" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between px-5 py-4 border-b border-forest-200 dark:border-forest-800">
                  ...
                  <button onClick={() => setFiltersOpen(false)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-forest-200 dark:border-forest-700 text-forest-500 hover:bg-forest-100 dark:hover:bg-forest-800 transition"><X size={15} /></button>
                </div>
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
                  {[
                    { label: "Field of study",    value: field,    setter: setField,    options: FIELDS },
                    { label: "Scholarship type",  value: type,     setter: setType,     options: TYPES },
                ...
                <div className="px-5 py-4 border-t border-forest-200 dark:border-forest-800 flex gap-3">
                  <button onClick={() => { clearFilters(); setFiltersOpen(false); }} className="flex-1 rounded-xl border border-forest-200 dark:border-forest-700 py-2.5 text-sm font-medium text-forest-600 dark:text-forest-300 hover:bg-forest-100 dark:hover:bg-forest-800 transition">Clear all</button>
                  <button onClick={() => setFiltersOpen(false)} className="flex-1 rounded-xl bg-forest-600 dark:bg-forest-500 py-2.5 text-sm font-medium text-white hover:bg-forest-700 transition">Show results</button>
                </div>
              </div>
            </div>
          )}
          ...
          <aside className="hidden lg:block shrink-0 w-56 space-y-4">
            <div className="rounded-2xl border border-forest-200 dark:border-forest-700 bg-white dark:bg-forest-900 p-4 space-y-4">
              <div className="flex items-center justify-between">
                ...
                {hasFilters && <button onClick={clearFilters} className="text-xs text-forest-500 hover:text-forest-800 dark:hover:text-white transition">Clear</button>}
              </div>
              {[
                { label: "Field of study",   value: field,    setter: setField,    options: FIELDS },
                { label: "Scholarship type", value: type,     setter: setType,     options: TYPES },
          ...
          <div className="flex-1 min-w-0">
                ...
                {[1,2,3,4,5,6].map((i) => (
                  <div key={i} className="rounded-2xl border border-forest-100 dark:border-forest-800 bg-white dark:bg-forest-900 p-5">
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2">
                      {[1,2,3,4].map((j) => <div key={j} className="h-3 rounded-lg bg-forest-100 dark:bg-forest-800 animate-pulse" />)}
                    </div>
            ...
            {error && (
              <div className="rounded-2xl border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-900/20 p-6 text-center">
                <p className="font-medium text-red-700 dark:text-red-400">Could not load scholarships</p>
                <p className="mt-1 text-sm text-red-600 dark:text-red-300">{error}</p>
              </div>
            )}
            {!loading && !error && scholarships.length === 0 && <EmptyState variant="empty" />}
            {!loading && !error && scholarships.length > 0 && filtered.length === 0 && <EmptyState variant="no-results" onClear={clearFilters} />}
            {!loading && !error && filtered.length > 0 && (
              <div className="grid gap-4 sm:grid-cols-2">
                {filtered.map((s, i) => <ScholarshipCard key={s.id || i} scholarship={s} index={i} />)}
              </div>
            )}
          </div>
        </div>
